...
function ForeningerView({
  ...
  const trin: {
    ...
    {
      navn: "Enheder i alle foreninger",
      antal: alleEnheder,
      maalt: true,
      note: `${raekker.length} foreninger i registret`,
    },
    {
      navn: "I foreninger vi vil købe i",
      antal: enheder,
      maalt: true,
      note: `${maal.length} foreninger med status målgruppe`,
    },
    {
      navn: "I størrelsen vi køber (20–80 kvm)",
      antal: null,
      maalt: false,
    ...
    {
      navn: "Har brugt boligberegneren",
      antal: beregnerLeads,
      maalt: true,
      note:
        beregnerLeads === 0
    ...
    <>
      <header className="mb-6">
        ...
        <p className="text-sm text-slate-500 mt-1">
          {raekker.length} foreninger · {maal.length} i målgruppen med{" "}
          {enheder.toLocaleString("da-DK")} enheder, hvoraf vi ejer {ejet} (
          {enheder ? ((100 * ejet) / enheder).toFixed(1) : "0"} %) · tal
          opdateret {fmtDato(senest as Date | null)}
        </p>
      </header>
        ...
        <section className="bg-white rounded-lg border border-slate-200 p-5 sm:p-6 mb-6">
          <div className="flex items-baseline justify-between flex-wrap gap-2 mb-5">
            ...
            <p className="text-xs text-slate-500">
              Hvert trin er en indsnævring af det forrige · procent viser
              frafald
            </p>
          </div>
          <div className="flex flex-col gap-2.5">
            {trin.map((t, i) => {
              ...
              const andelAfBasis =
                t.antal !== null && basis?.antal
                  ? (100 * t.antal) / basis.antal
                  : null;
              return (
                <div key={t.navn} className="flex items-center gap-3 sm:gap-4">
                  <div className="w-6 text-[11px] font-semibold text-slate-400 tabular-nums">
                    {i + 1}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-baseline justify-between gap-3 mt-1">
                      <span
                        className={`text-xs ${t.maalt ? "text-slate-500" : "text-amber-700"}`}
                      >
          ...
          <p className="text-xs text-slate-500 mt-5 pt-4 border-t border-slate-100 max-w-2xl leading-relaxed">
            De {ejet} købte kom gennem den hidtidige proces — brev, opkald og
            personlig kontakt — ikke gennem boligberegneren, som først lige er
            sat i luften. Trin 5 er derfor nul i dag, og det er dét tal, brev-
            og mailflowet skal flytte.
          </p>
        </section>
      )}
      ...
      {dbFejl ? (
        <div className="bg-white rounded-lg border border-amber-300 p-6">
          <h2 className="font-semibold text-slate-900 mb-1">
            Databasen mangler kolonnerne
          </h2>
          <p className="text-sm text-slate-600 mb-3">
            Kør migrering{" "}
            <code className="bg-slate-100 px-1.5 py-0.5 rounded">
              0007_foreninger_tragt
            </code>{" "}
            og derefter{" "}
            <code className="bg-slate-100 px-1.5 py-0.5 rounded">
              /api/admin/seed-foreninger
            </code>
            .
          </p>
          <pre className="text-xs bg-slate-50 border border-slate-200 rounded p-3 overflow-x-auto text-slate-600">
            {dbFejl}
          </pre>
        </div>
      ) : raekker.length === 0 ? (
        <div className="bg-white rounded-lg border border-slate-200 p-8 text-center">
          <p className="text-slate-600 text-sm">
            Ingen foreninger indlæst endnu. Kør{" "}
            <code className="bg-slate-100 px-1.5 py-0.5 rounded">
              POST /api/admin/seed-foreninger
            </code>{" "}
            for at hente dem fra regnearket.
          </p>
        </div>
      ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        ...
                        {r.statusReason && (
                          <div className="text-xs text-slate-500 mt-1 max-w-[22ch]">
                            {r.statusReason}
                          </div>
                        )}
                      </td>
      ...
      <p className="text-xs text-slate-500 mt-4 max-w-2xl leading-relaxed">
        Andel er hvor stor en del af foreningen vi ejer. Tallet kommer fra en
        krydsning af foreningens BFE-numre mod porteføljen og genberegnes ved
        hver indlæsning — i modsætning til regnearket, hvor det blev
        vedligeholdt i hånden og nåede at blive 27 for lavt.
      </p>
    </>
  );
}
