/**
 * Design F — Vores design + Zillow's info-led flow-struktur.
 *
 * Isolerer flow-spørgsmålet fra styling-spørgsmålet:
 * Bruger samme slate-900 design som /salg, men adopterer Zillow's
 * info-first flow (Zestimat + comparables FØR convert til offer).
 */
import type { Metadata } from 'next';
import Link from 'next/link';
import { PrototypeBanner } from '@/lib/components/PrototypeBanner';
import { MockFunnelZillowOurs } from '@/lib/components/MockFunnelZillowOurs';

export const metadata: Metadata = {
  robots: { index: false, follow: false },
};

export default function SalgFlowZillowPage() {
  return (
    <div className="min-h-screen bg-slate-50">
      <PrototypeBanner />
      <Header />
      <Hero />
      <MockFunnelZillowOurs />
      <Footer />
    </div>
  );
}
